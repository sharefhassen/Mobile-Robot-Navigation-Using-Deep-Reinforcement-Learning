        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for _ in range(total_timesteps):
                ...
                if can_sample and self.num_timesteps > self.learning_starts \
                    ...
                    self.td_errors_list.append(np.mean(td_errors))

                    if not self.epis_start_step:
                        self.epis_start_step = self.num_timesteps

                    if done:
                        average_td_error = sum(self.td_errors_list)/len(self.td_errors_list)
                        self.epis_start_step = None
                        self.td_errors_list = []

                        if not self.epis_start_step == self.learning_starts:
                            with open('logs/td_error.csv','a') as f:
                                f.write('\n'+str(average_td_error)) 
